...
# 第一节 SummaryWriter 与 add\_scalar
...
## 3\. add\_scalar 绘制曲线（记录标量：loss/acc/ 自定义函数）
...
|参数|含义|
|---|---|
|tag|曲线名称（字符串），同名会合并曲线|
|scalar\_value|Y 轴数值（损失、精度、i、2\*i 等）|
|global\_step|X 轴步数（迭代次数 /epoch）|
...
|参数|说明|
|---|---|
|tag|图片分组名称（train/test 分开）|
|img\_tensor|图像数据，支持 numpy 数组 /torch\.Tensor|
|global\_step|步数，拖动滑块切换不同 step 图片|
|dataformats|通道顺序（最容易报错）|
...
同一 tag 下多次调用 add\_image，网页出现滑块切换图片
...
- 多次调用、不同 global\_step：页面出现滑块，拖动切换不同迭代图片
